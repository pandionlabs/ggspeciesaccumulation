...
4.  numbers each row again (column y) This gives you the two columns
    needed to make a species accumulation curve: number of
    observations (x) and total number of species (y)
...
- geom:
  ...
  Override the default geom of "point" with others, like "smooth"
...
`stat_species_acc()` understands the following aesthetics. Required
...
species_key is the column specifying the species of each observation
record (row)
